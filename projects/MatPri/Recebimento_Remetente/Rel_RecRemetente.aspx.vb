            ...
            Else
                ...
                If Request.Browser.Browser = "IE" Then
                    If Session("Resolucao") = "768x1024" Then
                        ...
                        jscript += "window.open('Rep_RecRemetente.aspx','PaginaExibida', 'resizable=no, scrollbars=yes, menubar=no, width=1005, height=695, left= 5, top=5')"
                        jscript += "</script>"
                        ClientScript.RegisterClientScriptBlock([GetType](), "Cliente", jscript)
                    ElseIf Session("Resolucao") = "600x800" Then
                        jscript += "<script language='JavaScript'>"
                        jscript += "window.open('Rep_RecRemetente.aspx','PaginaExibida', 'resizable=no, scrollbars=yes, menubar=no, width=780, height=530, left= 5, top=5')"
                        jscript += "</script>"
                        ClientScript.RegisterClientScriptBlock([GetType](), "Cliente", jscript)
                    Else
                        jscript += "<script language='JavaScript'>"
